...
function ContestStatusEditor({ contestStatus, setContestStatus }) {
  ...
  const save = async () => {
    setSaving(true);
    // Re-fetch the latest saved status and only patch in the language tab
    // currently being edited. Writing the whole local `draft` (both en AND
    // es) would silently overwrite the *other* language with whatever
    // stale copy happened to be sitting in this browser session — which is
    // what was causing English to get clobbered when only Spanish was
    // edited and saved.
    const latest = (await sGet(STATUS_KEY)) || DEFAULT_STATUS;
    const merged = {
      status: draft.status,
      closedMessage: {
        ...DEFAULT_STATUS.closedMessage,
        ...latest.closedMessage,
        [msgLang]: draft.closedMessage?.[msgLang] ?? "",
      },
    };
    await sSet(STATUS_KEY, merged);
    setContestStatus(merged);
    setDraft(merged);
    setSaving(false);
    setSaved(true);
